...
func restrain(
	...
	changes.stage(
		Callable(self, "_attach_restraint").bind(
			target, rope, attached_restraint_id, actor.unit_id
		),
...
func _attach_restraint(
	...
	if not target.apply_restraint(attached_item.item_id):
		if attached_item != source_item:
			...
			source_item.quantity += 1
		return false
	# Record the captor separately so this remains compatible with older
	# TacticalUnitState scripts whose apply_restraint() accepted one argument.
	target.set("restraint_applied_by_unit_id", applied_by_unit_id)
