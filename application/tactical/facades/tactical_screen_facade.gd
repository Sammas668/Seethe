...
func pending_commit_envelope() -> MissionCommitEnvelope:
	if _mission_resolution_handler == null:
		return null
	return _mission_resolution_handler.pending_commit_envelope()
...
func player_last_seen_positions() -> Dictionary:
	...
	for squad: TacticalSquadState in state().get_squads_for_team(&"player"):
		for raw_unit_id: Variant in squad.last_seen_positions_by_unit_id.keys():
			result[raw_unit_id] = squad.last_seen_positions_by_unit_id[raw_unit_id]
	return result
