...
const ENEMY_TURN_INSTRUMENTATION_SCRIPT: Script = preload(
	"res://application/tactical/ai/enemy_turn_instrumentation.gd"
)
...
var _instrumentation: EnemyTurnInstrumentation
...
func _init() -> void:
	_instrumentation = ENEMY_TURN_INSTRUMENTATION_SCRIPT.new() as EnemyTurnInstrumentation
...
func performance_snapshot() -> Dictionary:
	...
	snapshot["activation_timing"] = _instrumentation.snapshot()
	return snapshot


func record_last_presentation_timing(presentation_usec: int) -> void:
	_instrumentation.record_presentation_timing(presentation_usec)
...
func _record_activation_timing_elapsed(
	...
	_instrumentation.record_activation_elapsed(
		unit_id,
		elapsed_usec,
		result,
		mode,
...
func _begin_activation_diagnostics(
	...
	_instrumentation.begin_activation({
...
func _add_activation_stage_time(stage: StringName, started_usec: int) -> void:
	_instrumentation.active_breakdown[stage] = int(
		_instrumentation.active_breakdown.get(stage, 0)
	) + maxi(0, Time.get_ticks_usec() - started_usec)
...
func _merge_planner_diagnostics() -> void:
	...
	_instrumentation.active_breakdown["planning"] = int(
		diagnostics.get("processing_usec", diagnostics.get("total_usec", 0))
	)
	_instrumentation.active_breakdown["planning_processing_usec"] = int(
		diagnostics.get("processing_usec", 0)
	)
	_instrumentation.active_breakdown["planning_wall_clock_usec"] = int(
		diagnostics.get("total_usec", 0)
	)
	_instrumentation.active_breakdown["planning_slices"] = int(
		diagnostics.get("planning_slices", 0)
	)
	_instrumentation.active_breakdown["target_discovery"] = int(
		diagnostics.get("target_discovery_usec", 0)
	)
	_instrumentation.active_breakdown["direct_attack_check"] = int(
		diagnostics.get("direct_attack_check_usec", 0)
	)
	_instrumentation.active_breakdown["reachable_field"] = int(
		diagnostics.get("reachable_field_usec", 0)
	)
	_instrumentation.active_breakdown["candidate_scoring"] = int(
		diagnostics.get("candidate_scoring_usec", 0)
	)
	_instrumentation.active_breakdown["targeted_melee_search"] = int(
		diagnostics.get("targeted_melee_search_usec", 0)
	)
	_instrumentation.active_breakdown["exact_geometry"] = int(
	...
	for key: StringName in [
		...
		_instrumentation.active_breakdown[key] = diagnostics.get(key, 0)
...
func _try_execute_hidden_auto_pass_batch() -> OperationResult:
	...
	_instrumentation.active_breakdown["plan_kind"] = &"hidden_auto_pass_batch"
	_instrumentation.active_breakdown["batch_size"] = units.size()
	_instrumentation.active_breakdown["hidden_auto_pass_batch"] = true
...
func _execute_standard_combat(
	...
	if plan_job == null:
		...
		if plan_job != null:
			_instrumentation.active_breakdown["handoff_warmup_reused"] = true
			_instrumentation.active_breakdown["chain_warmup_reused"] = handoff_was_chain
			_instrumentation.active_breakdown["handoff_warmup_processing_usec"] = (
				_handoff_warmup_processing_usec
			)
			_instrumentation.active_breakdown["handoff_warmup_slices"] = (
				_handoff_warmup_slices
			)
	else:
		_instrumentation.active_breakdown["contact_warmup_reused"] = true
		_instrumentation.active_breakdown["contact_warmup_processing_usec"] = (
			_contact_warmup_processing_usec
		)
		_instrumentation.active_breakdown["contact_warmup_slices"] = (
			_contact_warmup_slices
		)
	_instrumentation.active_breakdown["activation_start_to_plan_consumed_usec"] = maxi(
	...
	if _ability_service != null:
		...
		if has_start_work:
			...
			if (
				...
				_instrumentation.active_breakdown["cold_replan_after_warmup"] = true
		...
		_instrumentation.active_breakdown["activation_ability_scan_usec"] = maxi(
	...
	_instrumentation.active_breakdown["activation_support_scan_usec"] = maxi(
	...
	if plan_job != null and _warmup_perception_is_current(
		...
		_instrumentation.active_breakdown["perception_refresh_skipped"] = true
		_instrumentation.active_breakdown["perception_skip_reason"] = (
			&"warmup_revision_current"
		)
		_instrumentation.active_breakdown["perception_current_before_activation"] = true
	else:
		...
		if plan_job != null:
			...
			_warmup_invalidated_by_perception_count += 1
			_cold_replan_after_warmup_count += 1
			_instrumentation.active_breakdown["warmup_invalidated_by_perception"] = true
			_instrumentation.active_breakdown["cold_replan_after_warmup"] = true
	...
	_instrumentation.active_breakdown["activation_perception_gate_usec"] = (
...
func _capture_destination_visibility_job_diagnostics(job: RefCounted) -> void:
	if job == null:
		return
	_instrumentation.active_breakdown["prepared_visibility_cache_hit"] = (
		bool(job.get("valid"))
		and int(job.get("cache_misses")) == 0
	)
	_instrumentation.active_breakdown["destination_visibility_slices"] = int(
		job.get("processing_slices")
	)
	_instrumentation.active_breakdown["destination_visibility_cache_hits"] = int(
		job.get("cache_hits")
	)
	_instrumentation.active_breakdown["destination_visibility_cache_misses"] = int(
...
func _execute_planned_combat(
	...
	_instrumentation.active_breakdown["plan_kind"] = plan_kind
	if plan_kind in [
