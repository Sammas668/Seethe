...
func _item_colour() -> Color:
    ...
    if "bandage" in lower or "rations" in lower:
        return Color(0.25, 0.13, 0.11, 0.98)
    if "rope" in lower or "manacles" in lower or "lockpick" in lower:
        return Color(0.20, 0.17, 0.11, 0.98)
    if "grain crate" in lower or "sack" in lower:
        return Color(0.25, 0.18, 0.09, 0.98)
    if "bow" in lower or "spear" in lower or "axe" in lower:
