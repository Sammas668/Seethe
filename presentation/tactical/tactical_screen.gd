...
signal mission_result_ready(envelope: MissionCommitEnvelope)
...
func _on_mission_resolution_confirmed(
	...
	_set_status("Resolving extraction and preparing the campaign handoff...")
	...
	if mission_result == null:
		_set_status("Mission resolution returned no tactical result.")
		return
	_world_phase_in_progress = false
	...
	_set_status(result.message)
	_refresh_all_presentation()
	_emit_mission_result_ready(mission_result)


func _emit_mission_result_ready(_mission_result: MissionResult) -> void:
	var envelope: MissionCommitEnvelope = _facade.pending_commit_envelope()
	if envelope == null:
		_set_status("Mission resolution produced no campaign handoff envelope.")
		return
	mission_result_ready.emit(envelope)
...
func _on_mission_summary_continue() -> void:
	...
	_mission_resolution_window.hide()
	_board_view.set_input_enabled(false)
	_set_status("Mission result prepared. Returning to the outer mission flow.")
...
func _resolve_campaign_defeat_if_needed() -> void:
	...
	if mission_result != null:
		_mission_resolution_window.hide()
		_set_status("Campaign defeat resolved. Returning to the campaign shell.")
		_emit_mission_result_ready(mission_result)
...
func _resolve_tactical_defeat_if_needed() -> void:
	...
	if mission_result != null:
		_mission_resolution_window.hide()
		_set_status("Tactical defeat resolved. Returning to the campaign shell.")
		_emit_mission_result_ready(mission_result)
