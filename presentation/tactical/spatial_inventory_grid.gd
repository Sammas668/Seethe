...
func render_inventory_items(
        items: Array[TacticalInventoryItemState]
) -> void:
    _clear_items()

    for item: TacticalInventoryItemState in items:
        _add_item_control(
            item.item_id,
            item.display_name,
            item.footprint,
...
func render_ground_items(items: Array[TacticalItemState]) -> void:
    _clear_items()

    for ground_item: TacticalItemState in items:
        var position := _first_display_fit(ground_item.inventory_footprint)
        if position.x < 0:
            continue
...
func _can_drop_data(position: Vector2, data: Variant) -> bool:
    ...
    _hover_cell = _position_to_cell(position)
    queue_redraw()
...
func _drop_data(position: Vector2, data: Variant) -> void:
    ...
    var cell := _position_to_cell(position)
...
func _can_fit_at(
    ...
    if position.x < 0 or position.y < 0:
        return false
    if position.x + footprint.x > grid_width:
        return false
    if position.y + footprint.y > grid_height:
        return false
