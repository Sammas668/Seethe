...
const HOTFIX_5F_TESTS: Script = preload(
	"res://tests/integration/stage_4_7_hotfix_5f_enemy_movement_pipeline_tests.gd"
)
...
func _run() -> void:
	...
	failures.append_array(await HOTFIX_5F_TESTS.run(self))
	if failures.is_empty():
		print("Stage 4.7 Hotfix 5f enemy movement pipeline tests passed.")
		quit(0)
		return
	print("Stage 4.7 Hotfix 5f enemy movement pipeline tests failed:")
