...
# Feats in this list belong to the template's starting troop Tier. The
# character resolver removes this authored package and substitutes the persistent
# character's active Tier starting feats after Prestige.
...
# Stage 5.3 authored individual progression. Each entry targets one Level and
# may grant persistent stat adjustments, abilities and a bounded talent choice.
@export var level_progression_entries: Array[Dictionary] = []
...
func validate_definition() -> Array[String]:
	...
	var seen_progression_levels: Dictionary = {}
	for entry: Dictionary in level_progression_entries:
		var target_level: int = int(entry.get("level", 0))
		if target_level <= base_level:
			errors.append("Character template %s has progression at Level %d, not above base Level %d." % [id, target_level, base_level])
		elif seen_progression_levels.has(target_level):
			errors.append("Character template %s repeats progression Level %d." % [id, target_level])
		else:
			seen_progression_levels[target_level] = true
		if int(entry.get("xp_required", -1)) < 0:
			errors.append("Character template %s has a negative XP threshold at Level %d." % [id, target_level])
