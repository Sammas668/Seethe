#!/usr/bin/env python3
"""Apply the Stage 4.5g patch to an extracted Stage 4.5f Seethe project."""
...
MANIFEST = PATCH_PROJECT_ROOT / "STAGE_4_5G_PATCH_FILE_MANIFEST.txt"
...
def main() -> int:
    ...
    parser.add_argument("target", type=Path, help="Extracted Stage 4.5f seethe project root")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    target = args.target.resolve()
    if target == PATCH_PROJECT_ROOT.resolve():
        print("Refusing to apply the patch onto its own extracted patch directory.", file=sys.stderr)
        return 2
    ...
    missing_sources = [rel for rel in additions if not (PATCH_PROJECT_ROOT / rel).is_file()]
    if missing_sources:
    ...
    print(
        f"Stage 4.5g patch applied: {len(additions)} files copied, "
        f"{len(deletions)} obsolete paths removed."
    )
    return 0
