#!/usr/bin/env python3
"""Apply the Stage 4.7 Hotfix 5f1 patch to a Hotfix 5f project."""
...
def main() -> int:
    ...
    manifest = patch_root / "STAGE_4_7_HOTFIX_5F1_PATCH_FILE_MANIFEST.txt"
    ...
    release_marker = target / "STAGE_4_7_HOTFIX_5F_RELEASE_NOTES.txt"
    readme = target / "README_FIRST.txt"
    readme_text = readme.read_text(encoding="utf-8") if readme.is_file() else ""
    if not release_marker.is_file() and "HOTFIX 5f" not in readme_text:
        _fail(
            "Stage 4.7 Hotfix 5f baseline was not detected. "
            "Apply this patch only to the complete Hotfix 5f project."
        )
    ...
    print(f"Applied Stage 4.7 Hotfix 5f1 ({len(files)} files) to {target}")
    return 0
